...
static void     AddScriptToStack(const char* const filename)
{
    ...
    size = LoadFile(s_script->filename, (void**)&s_script->buffer);
...
bool IsScriptNonEmptyAndNotComment()
{
    char* scriptPtr = s_script->script_p;
    char* endPtr = s_script->end_p;

    // Check if script pointer has reached or exceeded the end pointer
    if (scriptPtr >= endPtr)
    {
        return false;
    }

    char currentChar = *scriptPtr;
