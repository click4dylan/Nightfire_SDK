...
#define LINK_ENTITY_TO_CLASS(mapClassName,DLLClassName) \
	extern "C" UTIL_DLLEXPORT void mapClassName( entvars_t2 *pev ); \
	void mapClassName( entvars_t2 *pev ) { GetClassPtr( (DLLClassName *)pev ); }
...
//
/*
#ifdef DEBUG //Dylan commented
	extern edict_t *DBG_EntOfVars(const entvars_t *pev);
	inline edict_t *ENT(const entvars_t *pev)	{ return DBG_EntOfVars(pev); }
#else*/
	inline edict_t *ENT(const entvars_t *pev)	{ return pev->pContainingEntity; }
	inline edict_t *ENT(const entvars_t2 *pev)	{ 
		return pev->pContainingEntity; 
	} //dylan added
	inline edict_t *ENT2(const entvars_t2 *pev)	{
		return pev->pContainingEntity;
	} //dylan added
//#endif
...
inline EOFFSET OFFSET(entvars_t2 *pev)	//dylan added	 		
{ 
#ifdef _DEBUG
	if ( !pev )
		ALERT( at_error, "Bad pev in OFFSET()\n" );
...
inline entvars_t2 *VARS2(edict_t *pent) //dylan added
{
	if (!pent)
		return NULL;

	return (entvars_t2*)&pent->v;
}
