...
typedef void(*StartMetaAudioFn)(unsigned long hEngineDLL, cl_exportfuncs_t* pExportFunc, cl_enginefunc_t* pEngineFuncs);
	...
	if (result)
	{
		...
		if (g_MetaAudioDllHinst)
		{
			...
			if (g_oStartMetaAudio && g_oShutdownMetaAudio)
				g_oStartMetaAudio(g_engineDllHinst, g_pExportFuncs, &g_oEngineFuncs);
		}
	}
