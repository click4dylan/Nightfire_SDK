...
typedef struct enginefuncs_s
{
	...
	void		(*pfnCVarSetString)			(const char *szVarName, const char *szValue); //this and the above was moved
	...
	int			(*pfnEntIsOnFloor)			(edict_t *e); //moved from above droptofloor
	...
	void		(*pfnGetBoneTransform)		( int iBone, /*matrix3x4_t INVALID!*/float pBoneToWorld ); //ADDED, float is wrong for bonetoworld
	...
	void		(*pfnUnknown3)				( void ); //added, i think it's something with gamespy/query
	void		(*pfnUnknown4)				( void ); //added
	void		(*pfnUnknown5)				( void ); //added
	void		(*pfnUnknown6)				( void ); //added

	//functions that don't exist in nightfire below
	
	//void		(*pfnCvar_DirectSet)		( struct cvar_s *var, const char *value ); //UNDONE, need to make a new wrapper, DirectSet still exists


	//void		(*pfnAddServerCommand)		( const char *cmd_name, void (*function) (void) ); DELETED
	//cvar_t	*(*pfnCVarGetPointer)		(const char *szVarName); //DELETED/MISSING?
	//void		(*pfnCvar_RegisterVariable) (cvar_t *variable); //DELETED
	//const char *(*pfnCmd_Args)				( void );		// these 3 added  //DELETED
	//const char *(*pfnCmd_Argv)				( int argc );	// so game DLL can easily //DELETED
	//int		 (*pfnCmd_Argc)				( void );		// access client 'cmd' strings //DELETED
	//void		(*pfnTraceSphere)			(const float *v1, const float *v2, int fNoMonsters, float radius, edict_t *pentToSkip, TraceResult *ptr);
	//void		(*pfnSaveSpawnParms)		(edict_t *ent); //DELETED


	// For voice communications, set which clients hear eachother.
	// NOTE: these functions take player entity indices (starting at 1).
	//qboolean	(*pfnVoice_GetClientListening)(int iReceiver, int iSender); //DELETED
	//qboolean	(*pfnVoice_SetClientListening)(int iReceiver, int iSender, qboolean bListen); //DELETED
	//const char *(*pfnGetPlayerAuthId)		( edict_t *e ); //this might be pfnGetPlayerWONid instead

	// PSV: Added for CZ training map
	//const char *(*pfnKeyNameForBinding)					( const char* pBinding ); //DELETED
