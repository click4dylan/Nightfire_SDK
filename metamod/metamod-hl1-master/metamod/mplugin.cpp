...
#define GET_FUNC_TABLE_FROM_PLUGIN(pfnGetFuncs, STR_GetFuncs, struct_field, API_TYPE, TABLE_TYPE, vers_pass, vers_int, vers_want) \
	...
	GET_FUNC_TABLE_FROM_PLUGIN(pfnGetNewDLLFunctions, 
			...
			*/
	if(meta_table.pfnGetNewDLLFunctions) {
		if (!newapi_table) {
			meta_new_dll_functions_t *newdllfuncs = ALLOC(meta_new_dll_functions_t);
			newapi_table = newdllfuncs;
			//newapi_table = (meta_new_dll_functions_t*) calloc(1, sizeof(meta_new_dll_functions_t));
		}
			if(meta_table.pfnGetNewDLLFunctions(newapi_table, &iface_vers)) {
			META_DEBUG(3, ("dll: Plugin '%s': Found %s", desc, "GetNewDLLFunctions"));
		}
		else {
			META_ERROR("dll: Failure calling %s in plugin '%s'", "GetNewDLLFunctions", desc);
			if(iface_vers != NEW_DLL_FUNCTIONS_VERSION)
				META_ERROR("dll: Interface version didn't match; expected %d, found %d", NEW_DLL_FUNCTIONS_VERSION, iface_vers);
		}
	}
	else {
		META_DEBUG(5, ("dll: Plugin '%s': No %s", desc, "GetNewDLLFunctions"));
		if (newapi_table)
			free(newapi_table);
		newapi_table=NULL;
	}
