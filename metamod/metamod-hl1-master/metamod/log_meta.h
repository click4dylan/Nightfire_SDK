...
extern int GetMetaDebug();

//#ifdef DEBUG
	#define META_DEBUG(level, args) \
	do { if(GetMetaDebug() < level) break; else ALERT(at_logged, "[META] (debug:%d) %s\n", level, UTIL_VarArgs args ); } while(0)
	//do { if((int)&*meta_debug.value < level) break; else ALERT(at_logged, "[META] (debug:%d) %s\n", level, UTIL_VarArgs args ); } while(0)
//#else
//#define META_DEBUG(level, args) \
	//do { break; } while (0)
//#endif
...
extern ConsoleVariable* meta_debug_ptr;
extern IConsoleVariable meta_debug;
