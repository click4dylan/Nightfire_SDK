...
void mutil_LogDeveloper(plid_t plid, const char *fmt, ...) {
	...
	static ConsoleVariable* var = CVAR_GET_POINTER("developer");
	if(!var || var->getValue() == 0)
		return;
...
// Jussi Kivilinna.
typedef void (*ENTITY_FN_NEW) (string_t *);
qboolean mutil_CallGameEntity(plid_t plid, const char *entStr, /*entvars_t*/string_t *pev) {
	plugin_info_t *plinfo;
	ENTITY_FN_NEW pfnEntity;
	
	plinfo=(plugin_info_t *)plid;
	META_DEBUG(8, ("Looking up game entity '%s' for plugin '%s'", entStr,
				plinfo->name));
	pfnEntity = (ENTITY_FN_NEW) DLSYM(GameDLL.handle, entStr);
	if(!pfnEntity) {
