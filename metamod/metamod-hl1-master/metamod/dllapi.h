...
extern BOOLEAN mm_DispatchBlocked( edict_t *pentBlocked, edict_t *pentOther );
...
extern BOOLEAN mm_DispatchBlockedTest( edict_t* pentBlocked, edict_t* pentOther );
...
extern void mm_ClientCommand( edict_t *pEntity, unsigned int numargs, const char **args );
...
extern void mm_Sys_Error(void);/*( const char *error_string );*/
...
extern void mm_PM_Move ( struct playermove_s *ppmove, /*int*/qboolean server );
...
extern BOOLEAN mm_Bot_GetParams(int blah1, char const** blah2, char const** returnplayername, char const** blah4);
extern void mm_Addbot(const char *difficulty, const char *botname, const char *team);
/*
extern void mm_OnFreeEntPrivateData(edict_t pEnt);
extern void mm_GameShutdown(void);
extern int mm_ShouldCollide(edict_t *pentTouched, edict_t *pentOther);
...
typedef BOOLEAN (*FN_DISPATCHBLOCKED) ( edict_t *pentBlocked, edict_t *pentOther );
...
typedef BOOLEAN (*FN_DISPATCHBLOCKEDTEST) (edict_t* pentBlocked, edict_t* pentOther);
...
typedef void (*FN_ENGCLIENTCOMMAND) ( edict_t *pEntity, const char *str, ... );
typedef void (*FN_CLIENTCOMMAND) (edict_t* pEntity, unsigned int numargs, const char** args);
...
typedef void(*FN_SYS_ERROR) (void);//( const char *error_string );

typedef void (*FN_PM_MOVE) ( struct playermove_s *ppmove, /*int*/qboolean server );
...
typedef BOOLEAN(*FN_ONSAVEGAME) (void);
typedef BOOLEAN(*FN_ONLOADGAME) (qboolean unknown);
typedef BOOLEAN(*FN_BOTGETPARAMS) (int blah1, char const** blah2, char const** returnplayername, char const** blah4);
typedef void (*FN_ADDBOT) (const char *difficulty, const char *botname, const char *team);
//Added 2005-08-11 (no SDK update)
