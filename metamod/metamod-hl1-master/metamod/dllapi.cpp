...
void mm_GameDLLInit(void) {
	META_DLLAPI_HANDLE_void(FN_GAMEINIT, pfnGameInit, ()); //NIGHTFIRE CRASHES HERE
	RETURN_API_void();
}
...
BOOLEAN mm_DispatchBlocked(edict_t *pentBlocked, edict_t *pentOther) {
	META_DLLAPI_HANDLE(BOOLEAN, 0, FN_DISPATCHBLOCKED, pfnBlocked, (pentBlocked, pentOther));
	RETURN_API();
}
...
BOOLEAN mm_DispatchBlockedTest(edict_t* pentBlocked, edict_t* pentOther) {
	META_DLLAPI_HANDLE(BOOLEAN, 0, FN_DISPATCHBLOCKEDTEST, pfnBlockedTest, (pentBlocked, pentOther));
	RETURN_API();
}
...
qboolean mm_ClientConnect(edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[ 128 ] ) {
	...
	META_DLLAPI_HANDLE(qboolean, TRUE, FN_CLIENTCONNECT, pfnClientConnect, (pEntity, pszName, pszAddress, szRejectReason));
	RETURN_API();
}
...
void  mm_ClientCommand(edict_t *pEntity, unsigned int numargs, const char** args) {
	if (numargs > 0 && strmatch(args[0], "meta"))
		client_meta(pEntity, numargs, args);
	
	META_DLLAPI_HANDLE_void(FN_CLIENTCOMMAND, pfnClientCommand, (pEntity, numargs, args));
	RETURN_API_void();
	//if (--CALL_API_count>0)
...
void mm_PM_Move (struct playermove_s *ppmove, /*int*/qboolean server) {
...
void mm_SetupVisibility(edict_t *pViewEntity, edict_t *pClient, unsigned char **pvs, unsigned char **pas) {
	META_DLLAPI_HANDLE_void(FN_SETUPVISIBILITY, pfnSetupVisibility, (pViewEntity, pClient, pvs, pas));
												//0x1c16820, 0x00000001, 0x1090163c, 0x0000019c
	RETURN_API_void();
}
...
BOOLEAN mm_OnSaveGame(void) {
	META_NEWAPI_HANDLE(BOOLEAN, 0, FN_ONSAVEGAME, pfnOnSaveGame, ());
	RETURN_API();
}
BOOLEAN mm_OnLoadGame(qboolean loading) {
	META_NEWAPI_HANDLE(qboolean, 0, FN_ONLOADGAME, pfnOnLoadGame, (loading));
	RETURN_API();
}
BOOLEAN mm_Bot_GetParams(int blah1, char const** blah2, char const** returnplayername, char const** blah4) {
	META_NEWAPI_HANDLE(BOOLEAN, 0, FN_BOTGETPARAMS, pfnBot_GetParams, (blah1, blah2, returnplayername, blah4));
	RETURN_API();
}
void mm_Addbot (const char *difficulty, const char *botname, const char *team) {
	META_NEWAPI_HANDLE_void(FN_ADDBOT, pfnAddbot, (difficulty, botname, team ));
	RETURN_API_void();
}
/*
...
static DLL_FUNCTIONS sFunctionTable = 
{
	...
	mm_DispatchBlockedTest,			//! pfnBlockedTest()
