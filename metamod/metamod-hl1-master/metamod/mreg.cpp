...
MRegCmd *MRegCmdList::find(const char *findname) {
	int i;
	for(i=0; i < endlist; i++) {
		if(mlist[i].function && !strcasecmp(mlist[i].function->getName(), findname))
				return(&mlist[i]);
	}
...
MRegCmd *MRegCmdList::add(const char *addname, IConsoleFunction* function) {
	MRegCmd *icmd;

	if (!function)
	{
		META_ERROR("Tried to add null function when adding reg cmd name '%s'", addname);
		RETURN_ERRNO(NULL, ME_BLANK);
	}
...
void MRegCmdList::disable(int plugin_id) {
	...
	for(i=0; i< size; i++) {
		if (mlist[i].plugid == plugin_id)
		{
			...
			if (mlist[i].function)
			{
				g_engfuncs.pfnUnregisterGame(mlist[i].function);
				delete mlist[i].function;
				mlist[i].function = nullptr;
			}
		}
	}
}
...
void MRegCmdList::show(int plugin_id) {
	...
	for(i=0; i < endlist; i++) {
		icmd = &mlist[i];
		if(icmd->plugid != plugin_id || !icmd->function)
			continue;
		META_CONS("   %s", icmd->function->getName());
		n++;
	}
...
MRegCvar *MRegCvarList::add(const char *addname) {
	...
	//    realloc (again, segv).
	icvar->data = (IConsoleVariable*) malloc(sizeof(IConsoleVariable));
	if(!icvar->data) {
...
void MRegCvarList::show(void) {
	...
	for(i=0; i < endlist; i++) {
		...
		if (icvar->registered_variable)
			snprintf(bval, sizeof(bval), "%f", icvar->registered_variable->getValue());
		else
			snprintf(bval, sizeof(bval), "unknown");
		...
		META_CONS(" [%*d] %-*s  %-*s  %*s  %s", 
				...
				sizeof(bval)-1, bval,
				icvar->registered_variable ? icvar->registered_variable->getValueString() : icvar->data->value);
