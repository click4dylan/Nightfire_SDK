...
IConsoleVariable meta_version = IConsoleVariable(CVAR_STRING, "metamod_version", "displays metamod version", VVERSION, FCVAR_SERVER);
...
int type; //int, bool, etc
	...
	int flags;
*/
#define EXTRAHEAPSIZEBYTES sizeof(DLL_FUNCTIONS)*20
...
void metamod_startup(void) {
	...
	/*
	void * test = calloc(1, sizeof(int));
	if (test) {
		printf("test\n");
	}
	void * test2 = calloc(1, sizeof(int));
	if (test2) {
		printf("test2\n");
	}
	void * test3 = calloc(1, sizeof(int));
	if (test3) {
		printf("test3\n");
	}

	DLL_FUNCTIONS* dlls = (DLL_FUNCTIONS*) calloc(1, sizeof(DLL_FUNCTIONS));
	dlls = (DLL_FUNCTIONS*) calloc(1, sizeof(DLL_FUNCTIONS));
	dlls = (DLL_FUNCTIONS*) calloc(1, sizeof(DLL_FUNCTIONS));*/
...
/*
	...
	size_t heapsize = HeapSize(procheap, NULL, heappointer); 
	HeapReAlloc(procheap, HEAP_ZERO_MEMORY | HEAP_REALLOC_IN_PLACE_ONLY | HEAP_GENERATE_EXCEPTIONS, heappointer, EXTRAHEAPSIZEBYTES);*/
	...
	// a debugger.
	/*if((int) CVAR_GET_STRING("developer") != 0) {
		sleep(10);
	}
	*/
	...
	// already set.
	ConsoleVariable* pDev = CVAR_GET_POINTER("developer");
	if (pDev && pDev->getValue() != 0 && meta_debug_ptr->getValueInt() == 0)
		meta_debug_ptr->setValueInt(3);
	...
	if(Config->debuglevel != 0)
		meta_debug_ptr->setValueInt(Config->debuglevel);
	...
	Engine.pl_funcs->pfnRegisterGame = meta_AddServerCommandNew;
	//Engine.pl_funcs->pfnCreateConsoleVariableGame = meta_CVarRegister;
...
*/
	if((pfn_getapinew = (GETNEWDLLFUNCTIONS_FN) DLSYM(GameDLL.handle, "GetNewDLLFunctions"))) {
			//static meta_new_dll_functions_t newdllfuncs;
		   meta_new_dll_functions_t * newdllfuncs = ALLOC(meta_new_dll_functions_t); //noel
			GameDLL.funcs.newapi_table = newdllfuncs; //noel
			//memset( reinterpret_cast<void*>(newdllfuncs), 0, sizeof(meta_new_dll_functions_t) );
			//GameDLL.funcs.newapi_table = (meta_new_dll_functions_t*) calloc(1, sizeof(meta_new_dll_functions_t));
			if(!GameDLL.funcs.newapi_table) {
				META_ERROR("malloc failed for gamedll struct_field: %s", "GetNewDLLFunctions");
			}
			else if(pfn_getapinew(GameDLL.funcs.newapi_table, &iface_vers)) {
				META_DEBUG(3, ("dll: Game '%s': Found %s", GameDLL.name, "GetNewDLLFunctions"));
			}
	}
...
*/
	...
	if(!found) {
		if((pfn_getapi = (GETENTITYAPI_FN) DLSYM(GameDLL.handle, "GetEntityAPI"))) {
			/*
			static DLL_FUNCTIONS dllfuncs;
			GameDLL.funcs.dllapi_table = &dllfuncs;
			memset( reinterpret_cast<void*>(&dllfuncs), 1, sizeof(DLL_FUNCTIONS) );
			*/
			DLL_FUNCTIONS * dllfuncs = ALLOC(DLL_FUNCTIONS); //noel
			//GameDLL.funcs.dllapi_table = (DLL_FUNCTIONS*) calloc(1, sizeof(DLL_FUNCTIONS));
			GameDLL.funcs.dllapi_table = dllfuncs; //noel
			//memset(reinterpret_cast<void*>(dllfuncs), 0, sizeof(DLL_FUNCTIONS));
			if(!GameDLL.funcs.dllapi_table) {
				META_ERROR("malloc failed for gamedll struct_field: %s", "GetEntityAPI");
			}
			else if((pfn_getapi(GameDLL.funcs.dllapi_table, &iface_vers))) {
				META_DEBUG(3, ("dll: Game '%s': Found %s", GameDLL.name, "GetEntityAPI"));
				found=1;
			}
			else {
				META_ERROR("dll: Failure calling %s in game '%s'", "GetEntityAPI", GameDLL.name);
				free(GameDLL.funcs.dllapi_table);
				//delete GameDLL.funcs.dllapi_table;
				//GameDLL.funcs.dllapi_table=NULL;
			}
		}
		else {
			META_DEBUG(5, ("dll: Game '%s': No %s", GameDLL.name, "GetEntityAPI"));
			GameDLL.funcs.dllapi_table=NULL;
		}
		/*
