...
class MRegCmd {
	...
	// data:
		int index;				// 1-based
	public:
		IConsoleFunction* function; // the function game will call
		//char *name;				// space is malloc'd
		REG_CMD_FN pfnCmd;		// pointer to the function we actually want to call
...
class MRegCmdList {
	...
	// functions:
		...
		MRegCmd *add(const char *addname, IConsoleFunction* function);
...
class MRegCvar {
	...
	// data:
		int index;				// 1-based
	public:
		IConsoleVariable *data;	// actual cvar structure, malloc'd
		int plugid;				// index id of corresponding plugin
		REG_STATUS status;		// whether corresponding plugin is loaded
		ConsoleVariable* registered_variable; // the resulting variable registered by the game
	// functions:
		void init(int idx);		// init values, as not using constructors
		mBOOL set(IConsoleVariable*src);
};
