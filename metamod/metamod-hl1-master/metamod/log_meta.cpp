...
int GetMetaDebug() {
	if (meta_debug_ptr)
		return meta_debug_ptr->getValueInt();
	return 0;
}
...
void META_DEV(const char *fmt, ...) {
	...
	if(g_engfuncs.pfnGetConsoleVariableGame) 
	{
		ConsoleVariable* var = g_engfuncs.pfnGetConsoleVariableGame("developer");
		if (!var || var->getValueInt() == 0)
			return;
	}
