...
extern ConsoleVariable* mm_CreateConsoleVariableGame(IConsoleVariable pCvar);
extern qboolean mm_DestroyConsoleVariableGame(IConsoleVariable *pCvar);
extern ConsoleVariable* mm_GetConsoleVariableGame(const char *szVarName);
extern void mm_RegisterGame(IConsoleFunction* pFunc);
extern void mm_UnregisterGame(IConsoleFunction* pFunc);
...
extern int mm_CheckBottom(edict_t *pEdict);
...
extern void mm_TraceLine(const float *v1, const float *v2, int mask, int collisionGroup, edict_t *pentToSkip, TraceResult *ptr);
extern void mm_TraceToss(edict_t *pent, edict_t *pentToIgnore, TraceResult *ptr);
extern int mm_TraceCharacterHull(edict_t *pEdict, const float *v1, const float *v2, int fNoMonsters, edict_t *pentToSkip, TraceResult *ptr);
extern void mm_TraceHull(const float *v1, const float *v2, int fNoMonsters, int blah, int hullNumber, edict_t *pentToSkip, TraceResult *ptr);
extern void mm_TraceModel(const float *v1, const float *v2, int hullNumber, int blah, edict_t *pent, TraceResult *ptr);
...
extern int mm_PointContents(const float *rgflVector, int brushflags);
extern BOOLEAN mm_WaterCheck(const float* pPos);
...
extern void mm_COM_Log(char *pszFile, char *fmt, ...);
extern const char*	mm_COM_FileBase(const char *name, char *dest, unsigned int size);

extern byte * mm_COM_LoadHeapFile(const char *filename, int *pLength);
extern const char*  mm_COM_ParseFile(const char *buffer, char *adr, unsigned int length);
extern char* mm_COM_Token();
...
extern void mm_GetGameDir(char *szGetGameDir, unsigned int length);
...
extern int mm_IsMapValid(const char *filename);
...
extern BOOLEAN mm_CheckVisibility( const edict_t *entity, unsigned char *pset );
...
extern int mm_engCreateInstancedBaseline( int classname, struct entity_state_s *baseline );
...
typedef int	(*FN_NUM_FOR_EDICT)			(const edict_t *e);
typedef int(*FN_GETPOINTILLUM)			(const float* pPos);
typedef int(*FN_CHECKBOTTOM)			(edict_t *pEdict);
typedef int (*FN_ALLOCATEDECAL)			(const char *name);
typedef BOOLEAN (*FN_GETBONECOORD)		(edict_t *ed, const char *bone, float* rgflOrigin, float* rgflAngles);
typedef void(*FN_COM_LOG)     (char *pszFile, char *fmt, ...);
typedef const char*(*FN_COM_FILEBASE)	(const char *name, char *dest, unsigned int size);
typedef const char* (*FN_COM_PARSEFILE)	(const char *buffer, char *adr, unsigned int length);
...
typedef BOOLEAN(*FN_WATERCHECK) (const float *pPos);
...
typedef void (*FN_TRACELINE) (const float *v1, const float *v2, int mask, int collisionGroup, edict_t *pentToSkip, TraceResult *ptr);
...
typedef void (*FN_TRACEHULL) (const float *v1, const float *v2, int fNoMonsters, int blah, int hullNumber, edict_t *pentToSkip, TraceResult *ptr);
typedef void (*FN_TRACEMODEL) (const float *v1, const float *v2, int hullNumber, int blah, edict_t *pent, TraceResult *ptr);
...
typedef void (*FN_ENGCLIENTCOMMAND) (edict_t *pEdict, const char *szFmt, ...);
typedef void (*FN_PARTICLEEFFECT) (const float *org, const float *dir, int red, int green, int blue, int count);
typedef void (*FN_LIGHTSTYLE) (int style, const char *val);
typedef int (*FN_DECALINDEX) (const char *name);
typedef int (*FN_POINTCONTENTS) (const float *rgflVector, int brushflags);
...
typedef ConsoleVariable* (*FN_CREATECONSOLEVARIABLEGAME) (IConsoleVariable pCvar);
typedef qboolean (*FN_DESTROYCONSOLEVARIABLEGAME) (IConsoleVariable* pCvar);
typedef float (*FN_CVarGetFloat) (const char *szVarName);
typedef int (*FN_CVarGetInt) (const char *szVarName);
typedef bool (*FN_CVarGetBool) (const char *szVarName);
typedef ConsoleVariable* (*FN_GETCONSOLEVARIABLEGAME) (const char *szVarName);
typedef const char * (*FN_CVarGetString) (const char *szVarName);
typedef void (*FN_REGISTERGAME) (IConsoleFunction* pFunc);
typedef void (*FN_CVarSetFloat) (const char *szVarName, float flValue);
typedef void (*FN_CVarSetInt) (const char *szVarName, int iValue);
typedef void (*FN_CVarSetBool) (const char *szVarName, bool bValue);
typedef void (*FN_UNREGISTERGAME) (IConsoleFunction* pFunc);
...
typedef void (*FN_ANIMATIONAUTOMOVE) (void/*const edict_t *pEdict, float flTime*/);
...
typedef byte * (*FN_COM_LOADHEAPFILE) (const char *filename, int *pLength);
...
typedef void (*FN_GETGAMEDIR) (char *szGetGameDir, unsigned int length);
...
typedef BOOLEAN (*FN_CHECKVISIBILITY) ( const edict_t *entity, unsigned char *pset );
...
typedef void (*FN_CMD_TOKENIZESTRING) (char *text);
typedef void (*FN_CMD_EXECUTESTRING) (char *text);
// Added in SDK 2.2:
