...
plugin_info_t Plugin_info = 
{
	...
	__DATE__,					// date
	"2GoldenBullet$",//"AMX Mod X Dev Team",		// author
	"http://www.nightfirepc.com", //"http://www.amxmodx.org",	// url
	"AMXX",						// logtag
...
qboolean C_ClientConnect_Post(edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[128])
{
	...
	if (!pPlayer->IsBot())
	{
		...
		if (a)
		{
			...
			if (g_auth_funcs.size())
			{
				...
				AUTHORIZEFUNC fn;
				//const char* authid = GETPLAYERAUTHID(pEntity); //DYLAN TOFIX
				//unsigned int authid = GETPLAYERAUTHID(pEntity); //DYLAN TOFIX
...
qboolean C_ClientConnect(edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[128])
{
	CPlayer* pPlayer = GET_PLAYER_POINTER(pEntity);
...
void C_ClientUserInfoChanged_Post(edict_t *pEntity, char *infobuffer)
{
	...
	if (pPlayer->ingame)
	{
		...
		if (g_auth_funcs.size())
		{
			...
			AUTHORIZEFUNC fn;
			//const char* authid = GETPLAYERAUTHID(pEntity); //DYLAN TOFIX
			//unsigned int authid = GETPLAYERAUTHID(pEntity);
			for (iter=g_auth_funcs.begin(); iter!=end; iter++)
			{
				fn = (*iter);
				fn(pPlayer->index, authid);
...
void C_ClientCommand(edict_t *pEntity, unsigned int numargs, const char** args)
{
	...
	if (IS_DEDICATED_SERVER())
	{
		if (cmd && stricmp(cmd, "amxx") == 0)
		{
			...
			len += sprintf(&buf[len], "         Dylan \"2GoldenBullet$\" Hughes, Ignacio Belhot\n");
...
void C_TraceLine_Post(const float *v1, const float *v2, int fNoMonsters, int Unknown, edict_t *e, TraceResult *ptr)
{	
