...
void Cvar_DirectSet_Custom(struct cvar_s *var, const char *value, IRehldsHook_Cvar_DirectSet *chain = nullptr)
{
	...
	if (info->bound.hasMin || info->bound.hasMax) // cvar_s doesn't have min/max mechanism, so we check things here.
	{
		...
		{
			ConsoleVariable* pVar = g_engfuncs.pfnGetConsoleVariableGame(var->name);
			if (pVar)
				pVar->setValueFloat(fvalue);
			return;
		}
	}
...
CvarInfo* CvarManager::CreateCvar(const char* name, int type, const char* value, const char* plugin, int pluginId, int flags,
								  const char* helpText)
{
