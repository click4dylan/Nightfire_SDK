...
#define  UINT32_MAX  ((unsigned long)~0)
...
inline T DWordSwapC( T dw )
{
   ...
   temp  =   *((unsigned long *)&dw) 				>> 24;
   temp |= ((*((unsigned long *)&dw) & 0x00FF0000) >> 8);
   temp |= ((*((unsigned long *)&dw) & 0x0000FF00) << 8);
   temp |= ((*((unsigned long *)&dw) & 0x000000FF) << 24);
...
inline unsigned long BigDWord( unsigned long val )	{ int test = 1; return ( *(char *)&test == 1 ) ? DWordSwap( val ) : val; }
...
inline unsigned long LittleDWord( unsigned long val )	{ int test = 1; return ( *(char *)&test == 1 ) ? val : DWordSwap( val ); }
