...
void ClientDisconnect(edict_t *pEntity)
{
	...
	if ( !plinfo[id].bSupportsDynamicLights )
		glinfo.NumPlayersWithoutDynamicLightSupport--;
	else
		plinfo[id].bSupportsDynamicLights = false;

	DeleteCamera(pEntity);
	plinfo[id].bSpectating = false;
	DisableFreeLook(pEntity);
	plinfo[id].flLastSpectateButtonPress = 0;
	plinfo[id].flNextSpectateUpdate = 0;
	plinfo[id].bCodeEnabledSpectator = false;
	plinfo[id].vecOriginBeforeSpectating = Vector(0.0f, 0.0f, 0.0f);
	plinfo[id].pSpectateLastTargetEdict = nullptr;
	plinfo[id].pSpectateTargetEdict = nullptr;
	plinfo[id].pCameraEdict = nullptr;
...
BOOLEAN ClientConnect(edict_t *pEntity, const char *pszName, const char *pszAddress, char szRejectReason[128])
{
	int id = ENTINDEX(pEntity);
	...
	plinfo[id].iRenderMode = 0;
	plinfo[id].fRenderAmt = 0;
	plinfo[id].bSupportsDynamicLights = true; //initialize to true so we can increment the number of players that don't support them when they send us their gamever
	DeleteCamera(pEntity);
	plinfo[id].bSpectating = false;
	DisableFreeLook(pEntity);
	plinfo[id].flLastSpectateButtonPress = 0;
...
void ServerDeactivate()
{
	...
	glinfo.bCheckLights = false;
	glinfo.fNextLights = 0;
	glinfo.NumPlayersWithoutDynamicLightSupport = 0;
