...
{
	int index = ENTINDEX(pEntity);
	//MF_ExecuteForward(PlayerPreThinkForward, (cell)index);
	
	//if ( gpGlobals->time - flLastThinkTime >= 0.0015625 ) { //1 sec / 32 players * 20 updates per sec
		//flLastThinkTime = gpGlobals->time;
		if ( plinfo[index].bSpectating ) {
			if ( gpGlobals->time >= plinfo[index].flNextSpectateUpdate ) {
				plinfo[index].flNextSpectateUpdate = gpGlobals->time + 0.022222;
				CheckCameraButtons(index, pEntity);
				UpdateCamera(index, pEntity);
				StripWeapons(pEntity, 1);
			}
		}
		/*
		client_t *pclient = EDICT_TO_CLIENT(pEntity);
		int actualnetchanack = (int)pclient + 0x44;

		client_frame_t *frame;
		int mask = SV_UPDATE_MASK;
		frame = &pclient->frames[ pclient->netchan.incoming_acknowledged & SV_UPDATE_MASK ];

		int frames = (int)&pclient->frames;
		int wrongframes = (int)pclient + 0x3574;

		int edict = (int)&pclient->edict; //real

		int sz = edict - frames; //5636

		int szofcft = sizeof(client_frame_t);

		int wrong = (int)&pclient->netchan.incoming_acknowledged;
		double connection_time = pclient->netchan.rate;
		double ping = pclient->frames->senttime;
		int actual = (int)pclient + 0x44;
		int size = sizeof(netchan_t);
		int wrong2 = (int)&pclient->frames;
		int difference = wrong - (int)pclient;
		*/
	//}
	...
	if(plinfo[ENTINDEX(pEntity)].pViewEnt) {
		...
		switch(plinfo[ENTINDEX(pEntity)].iViewType) {
			case CAMERA_3RDPERSON:
				TRACE_LINE(vecSrc, vecSrc - (vecAiming * 128), 0, ignore_monsters, ENT(pEntity), &tr);
				SET_VIEW(pEntity, pCamEnt);
				pCamEnt->v.origin = tr.vecEndPos;
				pCamEnt->v.angles = pEntity->v.v_angle;
				break;
			case CAMERA_UPLEFT:
				TRACE_LINE(vecSrc, vecSrc - ((vecAiming * 32) - ((gpGlobals->v_right * 15) + (gpGlobals->v_up * 15))), 0/*dylan*/, ignore_monsters, ENT(pEntity), &tr);
				SET_VIEW(pEntity, pCamEnt);
				pCamEnt->v.origin = tr.vecEndPos;
				pCamEnt->v.angles = pEntity->v.v_angle;
				break;
			case CAMERA_TOPDOWN:
				TRACE_LINE(vecSrc, vecSrc + Vector(0,0,2048), 0, dont_ignore_monsters, ENT(pEntity), &tr);
				SET_VIEW(pEntity, pCamEnt);
				pCamEnt->v.origin = tr.vecEndPos;
...
void ClientCommand( edict_t *pEntity, int argl, const char *str ) {
	//argl is 1 extra than the real value with ClientCommand/subtract by 1 to get actual value
	const char *actualstring = *(const char**)str; //str is a double pointer in nightfire
	if ( actualstring ) {
		if ( !strcmpi(actualstring, "spectate") ) {
			if ( !plinfo[ENTINDEX(pEntity)].bCodeEnabledSpectator ) //Player can only change spectator modes if server hasn't forced them there
				SpectateConsoleCommand(pEntity, argl);
		} else if ( !strcmp(actualstring, "codeexitspectate") ) {
			//Server forcing player to exit spectate
			if ( plinfo[ENTINDEX(pEntity)].bSpectating ) {
				DeleteCamera(pEntity);
				plinfo[ENTINDEX(pEntity)].bSpectating = false;
				plinfo[ENTINDEX(pEntity)].iSpectateMode = MODE_THIRDPERSON;
				plinfo[ENTINDEX(pEntity)].bCodeEnabledSpectator = false;
				ChangePlayerTeamCTF(C_BasePlayer::Instance(pEntity), "Spectators", plinfo[ENTINDEX(pEntity)].iLastTeam/*GetTeamchoice(C_BasePlayer::Instance(pEntity))*/);// , true);
			}
		} else if ( !strcmp(actualstring, "codebeginspectate") ) {
			//Server forcing player to spectate
			if ( !plinfo[ENTINDEX(pEntity)].bSpectating ) {
				plinfo[ENTINDEX(pEntity)].bCodeEnabledSpectator = true;
				SpectateConsoleCommand(pEntity, argl);
			}
		}
	}
...
void ClientUserInfoChanged( edict_t *pEntity, char *infobuffer ) {
	DLight_HandleClientUserInfoChanged(pEntity, infobuffer);
	RETURN_META(MRES_IGNORED);
}