...
static DLL_FUNCTIONS g_EntityAPI_Table = 
{
	...
	NULL,
#endif
#ifdef UNKNOWN_GAMEDLL1 //DYLAN ADDED
	FN_UNKNOWN_GAMEDLL1,
#else
	NULL,
#endif
...
static enginefuncs_t g_EngineFuncs_Table = 
{
	...
	NULL,
#endif
#ifdef FN_CVarGetFloat_Old
	FN_CVarGetFloat_Old,
#else
	NULL,
#endif
#ifdef FN_CVarGetString_Old
	FN_CVarGetString_Old,
#else
	NULL,
#endif
#ifdef FN_CVarSetFloat_Old
	FN_CVarSetFloat_Old,
#else
	NULL,
#endif
#ifdef FN_CVarSetString_Old
	FN_CVarSetString_Old,
#else
	NULL,
#endif
	...
	NULL,
#endif
#ifdef FN_Unknown1_FixedLight
	FN_Unknown1_FixedLight,
#else
	NULL,
#endif
	...
	NULL,
#endif
#ifdef FN_GetBoneTransform1
	FN_GetBoneTransform1,
#else
	NULL,
#endif
	...
	NULL,
#endif
#ifdef FN_LoadFileForMe2
	FN_LoadFileForMe2,
#else
	NULL,
#endif
#ifdef FN_COM_FileBase
	FN_COM_FileBase,
#else
	NULL,
#endif
	...
	NULL,
#endif
#ifdef FN_CheckParm //dylan added, wasn't in original at all
	FN_CheckParm,
#else
	NULL,
#endif
	...
	NULL,
#endif
#ifdef FN_Cmd_TokenizeString //dylan added, didn't exist in original
	FN_Cmd_TokenizeString,
#else
	NULL,
#endif
#ifdef FN_Cmd_ExecuteString //dylan added, didnt exist in original
	FN_Cmd_ExecuteString,
#else
	NULL,
#endif
...
{
	LOG_DEVELOPER(PLID, "called: GetEntityAPI2; version=%d\n", *interfaceVersion);
	if(!pFunctionTable) {
...
{
	LOG_DEVELOPER(PLID, "called: GetEntityAPI2_Post; version=%d\n", *interfaceVersion);
	if(!pFunctionTable) {
...
C_DLLEXPORT	int	GetEngineFunctions(enginefuncs_t *pengfuncsFromEngine, int *interfaceVersion)
{
	...
	/*
	else if((int)interfaceVersion != (int)ENGINE_INTERFACE_VERSION) {
		LOG_ERROR(PLID, 
				"GetEngineFunctions version mismatch; requested=%d ours=%d", 
				(int)interfaceVersion, (int)ENGINE_INTERFACE_VERSION);
		// Tell metamod what version we had, so it can figure out who is 
		// out of date.
		*interfaceVersion = ENGINE_INTERFACE_VERSION;
		return(FALSE);
	...
	*/
	...
	memset(&g_EngineFuncs_Table, 0, sizeof(enginefuncs_t));
...
{
	LOG_DEVELOPER(PLID, "called: GetEngineFunctions_Post; version=%d", 
			*interfaceVersion);
	if(!pengfuncsFromEngine) {
		LOG_ERROR(PLID, 
				"GetEngineFunctions_Post called with null pengfuncsFromEngine");
		return(FALSE);
	}
	/*
	else if((int)interfaceVersion != (int)ENGINE_INTERFACE_VERSION) {
...
*/
//NEW_DLL_FUNCTIONS *pHookedNewDllFunctions = &g_NewFuncs_Table;

C_DLLEXPORT int GetNewDLLFunctions(NEW_DLL_FUNCTIONS *pNewFunctionTable, int *interfaceVersion) 
{
	printf("called: GetNewDLLFunctions; version=%d\n", *interfaceVersion);
	...
	if(!pNewFunctionTable) {
		printf("GetNewDLLFunctions called with null pNewFunctionTable");
		return(FALSE);
	}
	/*
	else if(*interfaceVersion != NEW_DLL_FUNCTIONS_VERSION) {
		printf("GetNewDLLFunctions version mismatch; requested=%d ours=%d", *interfaceVersion, NEW_DLL_FUNCTIONS_VERSION);
		//! Tell engine what version we had, so it can figure out who is out of date.
		*interfaceVersion = NEW_DLL_FUNCTIONS_VERSION;
		return(FALSE);
	}
...
{
	LOG_DEVELOPER(PLID, "called: GetNewDLLFunctions_Post\n; version=%d", *interfaceVersion);
	if(!pNewFunctionTable) {
