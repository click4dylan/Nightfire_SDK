...
void CL_SendMove(cmd_t* cmd, sizebuf_t *buf, byte* data)
{
	...
	if (!IsDedicatedServer())
		...
		else
		{
			...
			{
				from = &CMDS->commands[CL_UPDATE_MASK & (g_pcls->netchan.outgoing_sequence - numcmds)].cmd;
...
cl_entity_t * (*GetLocalPlayer) (void) = (cl_entity_t*(*)(void))0x4302BCD0;

void CL_Move(void){
	sizebuf_t buf;
	byte data[MAX_USERCMD_BUFFER];

	if (*bNotFinalTick || g_pcls->state == ca_dedicated || g_pcls->state == ca_disconnected || g_pcls->state == ca_connecting)
		return;
	...
	if (g_pcls->state == ca_active || g_pcls->state == ca_uninitialized || g_pcls->state == ca_connected)
	{
		...
		if (GetAsyncKeyState(VK_SPACE))
		{
			...
			if (pPlayer)
			{
				...
				{
					if (pPlayer->curstate.flFallVelocity != 1.0f)//GetFlags() & FL_ONGROUND)
					{
						bLastJumped = true;
						bShouldFake = true;
					}
