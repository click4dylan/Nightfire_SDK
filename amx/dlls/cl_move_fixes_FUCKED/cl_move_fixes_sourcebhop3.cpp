...
void CL_Move(void){
	...
	//if (*bNotFinalTick)
		//return; //Until we can get goldsrc to behave like source, don't hold back dup ticks
	...
	if (g_pcls->state == ca_active || g_pcls->state == ca_uninitialized || g_pcls->state == ca_connected)
	{
		...
		else
		{
			if (*bNotFinalTick)
			{
				CMDS->commands[g_pcls->netchan.outgoing_sequence & CL_UPDATE_MASK].cmd.msec = 0;
			}
