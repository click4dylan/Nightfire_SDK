...
//struct netadr_t;
//struct sizebuf_t;
int* net_drop = (int*)0x448C27B0;
netadr_t* net_from = (netadr_t*)0x448F4A60;
sizebuf_t *net_message = (sizebuf_t*)0x448EA9B8;
int* msg_readcount = (int*)0x4348226C;

ExtendedClient_t g_ExtendedClient[32];

#if 1

char gDownloadFile[256];

cvar_t_small sv_filetransfermaxsize_s = { CVAR_INT, "sv_filetransfermaxsize", "", "10485760", 0 };
int* sv_filetransfermaxsize = nullptr;
...
cvar_t *net_chokeloopback = (cvar_t*)0x448C27CC;

qboolean Netchan_CanPacket(netchan_t *chan)
{
	// Never choke loopback packets.
	if (net_chokeloopback->GetFloat() == 0.0f && chan->remote_address.type == NA_LOOPBACK)
	{
...
cvar_t* net_showpackets = (cvar_t*)0x448C28A4;
...
void Netchan_Transmit(netchan_t *chan, int length, byte *data)
{
	...
	if (net_showpackets->GetFloat() != 0.0f && net_showpackets->GetFloat() != 2.0f) {
...
cvar_t* net_showdrop = (cvar_t*)0x448C29E8;
...
qboolean Netchan_Process(netchan_t *chan)
{
	...
	*net_drop = sequence - (chan->incoming_sequence + 1);
	if (net_drop > 0 && net_showdrop->GetFloat() != 0.0)
	{
...
void Netchan_CreateFileFragmentsFromBuffer(qboolean server, netchan_t *chan, const char *filename, unsigned char *uncompressed_pbuf, int uncompressed_size)
{
	...
	while (remaining > 0)
	{
		...
		if (!buf)
		{
			...
			{
				Sys_Error("Couldn't allocate fragbuf_t");
...
#ifdef REHLDS_FIXES
{
	...
	if (!FS_FileExists(filename))
		return FALSE;
	if (FS_FileSize(filename, false) > *sv_filetransfermaxsize)
		return FALSE;
...
#endif // REHLDS_FIXES
{
	...
	if (compressedFileTime >= FS_GetFileTime(filename) && (hfile = FS_Open(compressedfilename, "rb")))
	{
		...
		if (uncompressed_size > *sv_filetransfermaxsize)
		{
			FS_Close(hfile);
	...
	else
	{
		...
		filesize = FS_Size(hfile);
		if (filesize > *sv_filetransfermaxsize)
		{
			FS_Close(hfile);
			Con_Printf("Warning:  File %s is too big to transfer from host %s\n", filename, NET_AdrToString(chan->remote_address));
			return 0;
...
qboolean Netchan_CopyNormalFragments(netchan_t *chan)
{
	...
	if (overflowed)
	{
		...
		{
			Con_Printf("%s: Incoming overflowed from %s\n", __FUNCTION__, g_psvs->clients[chan->player_slot - 1].name);
		}
...
cvar_t *net_drawslider = (cvar_t*)0x448C28EC;
...
NOXREF void Netchan_UpdateProgress(netchan_t *chan)
{
	...
	//scr_downloading->SetFloat(-1.0f);
	gDownloadFile[0] = 0;

	if (net_drawslider->GetFloat() != 1.0f)
	{
	...
	for (i = MAX_STREAMS - 1; i >= 0; i--)
	{
		if (chan->incomingbufs[i])
		{
			...
			if (i == FRAG_FILE_STREAM)
			{
				...
				*out = '\0';
				if (strlen(sz) > 0 && (*sz != '!' || !strncmp(sz, "!ModuleC.dll", 11)))
				{
					strncpy(gDownloadFile, sz, 255);
					gDownloadFile[255] = 0;
				}
			}
		}
	...
	//scr_downloading.SetFloat( bestpercent);
}
