...
//		if ( hMod ) {
//			FARPROC test = GetProcAddress(hMod, /*lpProcName*/ "bomb_target");
...
void OnModuleInitialize(void) {
	cvar_t startmoney = {CVAR_INT, "mp_startmoney", "Money start", (int*)"800", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t roundtime = {CVAR_FLOAT, "mp_roundtime", "Round Time in minutes", (int*)"5.0", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t freezetime = {CVAR_FLOAT, "mp_freezetime", "Freeze Time", (int*)"7.0", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t warmuptime = {CVAR_FLOAT, "mp_warmuptime", "Warmup Time", (int*)/*"240.0"*/"3.0", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t maxrounds = {CVAR_INT, "mp_maxrounds", "Max Rounds", (int*)"31", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t round_restart_delay = {CVAR_FLOAT, "mp_round_restart_delay", "Delay after a round ends before a new one begins", (int*)"7.0", FCVAR_EXTDLL | FCVAR_SERVER };
	cvar_t c4timer = {CVAR_FLOAT, "mp_c4timer", "Time in seconds before C4 explodes", (int*)"45.0", FCVAR_EXTDLL | FCVAR_SERVER };
...
void OnStartFrame(void) {
	if ( IsTimeToThink() ) {
		...
		if ( !glinfo.bWarmupComplete ) {
			...
			//Warmup Think
			glinfo.flClockSeconds = fmax(0, glinfo.flWarmupTimeEnd - gpGlobals->time);
...
void GameEndThink(void) {
	...
	glinfo.flClockSeconds = fmax(0, enddelay - flTimeSinceGameEnd);
...
void RoundEndThink(void) {
	...
	glinfo.flClockSeconds = fmax(0, restartdelay - flTimeSinceRoundEnd);
...
void RoundPreStartThink(void) {
	glinfo.flClockSeconds = fmax(0, glinfo.flRoundPreStartTimeEnd - gpGlobals->time);
	FreezePlayers();
...
void RoundThink(void) {
	...
	glinfo.flClockSeconds = fmax(0, glinfo.flRoundTimeEnd - gpGlobals->time);
	if ( gpGlobals->time >= glinfo.flRoundTimeEnd ) {
		glinfo.flNextThink = 0;
		glinfo.flNextTimeDrawClock = 0;
