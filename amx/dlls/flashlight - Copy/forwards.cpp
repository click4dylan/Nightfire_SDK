...
void PlayerPreThink(edict_t *pEntity)
{
	...
	OnPlayerPreThink(pEntity, index);
...
void ClientCommand( edict_t *pEntity, int unknown, const char *str ) {
	const char *actualstring = *(const char**)str; //str is a double pointer in nightfire

	if ( !strcmpi(actualstring, "flashlight") ) { //todo: impulse 100
		ToggleFlashlight(pEntity);
	}
	RETURN_META(MRES_IGNORED);
}
...
{
	int retValue = callOnAddToFullPack(state, e, ent, host, hostflags, player, pSet);
	if (retValue == 1)
		RETURN_META_VALUE(MRES_OVERRIDE, 1);
	else if (!retValue)
		RETURN_META_VALUE(MRES_OVERRIDE, 0);

	RETURN_META_VALUE(MRES_IGNORED, 0); //-1
}