...
struct PlayerInfo {
	float flNextThink;
	float flTimeToKickForOldVersion;
	float flTimeToRemindPlayerForOldVersion;
	bool bOldVersion;
	int iSpeakFlags;
	edict_t *pViewEnt;
	int iViewType;
	int iRenderMode;
	float fRenderAmt;
	bool bSupportsDynamicLights;
	int iMoney;
	bool bFakeClient;
	bool bCodeInitiatedSpectate;
	bool bAwaitingSpectate;
	float flTimeBeginSpectate;
	float flTimeToSpawn;
	int iTeam;
};
...
struct GlobalInfo {
	...
	bool bCheckLights;
	unsigned int NumPlayersWithoutDynamicLightSupport;
	int iNumPlayers;
	int iNumPlayersAliveMI6;
	int iNumPlayersAlivePhoenix;
	float flNextThink;
	int iRoundNumber;
	float flRoundTimeEnd;
	bool bWarmupComplete;
	bool bWarmupStarted;
	float flWarmupTimeEnd;
	bool bRoundEndThink;
	bool bRoundThink;
	bool bGameEndThink;
	float flGameTimeEnd;
	bool bRoundPreStartThink;
	float flRoundPreStartTimeEnd;
	bool bAllowPlayerSpawn;
	float flNextTimeDrawClock;
	float flClockSeconds;
	int *clBombPlanter;
	int *clBombCarrier;
	edict_t *pBomb;
	bool bReceivedBombSites;
	float flTimeToCheckBombSites;
	edict_t *pBombSiteA;
	float flBombSiteARadius;
	edict_t *pBombSiteB;
	int iBombSitePlantingAt;
	float flBombSiteBRadius;
	int iNumBombSites;
	bool bBombExplodeThink;
	bool bBombExploded;
	int *clBombDefuser;
	float flPrevDefuserMaxSpeed;
	float flPrevPlanterMaxSpeed;
	float flBombExplodeTime;
	bool bBombDefused;
	float flBombDefuseTime;
	float flLastDefuseTime;
	int iRoundsWonMI6;
	int iRoundsWonPhoenix;
	bool bBombStartedPlanting;
	float flBombPlantTime;
	bool bPlayedWarningSound;
	float flNextBeepSound;
	int g_sModelIndexFireball;
	bool bForcePlayerAliveCheck;
	int iNumPlayersAwaitingSpectate;
	int iNumPlayersAwaitingSpawn;
	float flNextTimeForceAliveCheck;
};
