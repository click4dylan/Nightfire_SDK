...
cvar_t* mp_startmoney = nullptr;
cvar_t* mp_roundtime = nullptr;
cvar_t* mp_freezetime = nullptr;
cvar_t* mp_warmuptime = nullptr;
cvar_t* mp_maxrounds = nullptr;
cvar_t* mp_round_restart_delay = nullptr;
cvar_t* mp_c4timer = nullptr;
...
void OnModuleInitialize(void) {
	if (!mp_startmoney)
	{
		...
		mp_startmoney = (cvar_t*)CVAR_REGISTER(startmoney);
		mp_roundtime = (cvar_t*)CVAR_REGISTER(roundtime);
		mp_freezetime = (cvar_t*)CVAR_REGISTER(freezetime);
		mp_warmuptime = (cvar_t*)CVAR_REGISTER(warmuptime);
		mp_maxrounds = (cvar_t*)CVAR_REGISTER(maxrounds);
		/*
		int test;
		__asm {
		...
		mp_round_restart_delay = (cvar_t*)CVAR_REGISTER(round_restart_delay);
		mp_c4timer = (cvar_t*)CVAR_REGISTER(c4timer);
...
void OnClientConnect(edict_t *pEntity) {
	...
	plinfo[index].iMoney = mp_startmoney->GetInt();
}
	...
	if ( IsTimeToThink() ) {
		...
		if ( !glinfo.bWarmupComplete ) {
			...
			if ( !glinfo.bWarmupStarted ) {
				...
				glinfo.flWarmupTimeEnd = gpGlobals->time + mp_warmuptime->GetFloat();
			}
...
void OnBombPlant(edict_t *pBombSite) {
	...
	glinfo.flBombExplodeTime = gpGlobals->time + mp_c4timer->GetFloat();
...
void GameEndThink(void) {
	...
	float enddelay = mp_round_restart_delay->GetFloat();
	glinfo.flClockSeconds = max(0.0f, enddelay - flTimeSinceGameEnd);
...
void RoundEndThink(void) {
	...
	float restartdelay = mp_round_restart_delay->GetFloat();
	glinfo.flClockSeconds = max(0.0f, restartdelay - flTimeSinceRoundEnd);
...
void RoundEnd(void) {
	...
	if ( glinfo.iRoundNumber > mp_maxrounds->GetInt() - 1 ) { // > because we start from 1
...
void RoundPreStart(void) {
	...
	glinfo.flRoundPreStartTimeEnd = gpGlobals->time + mp_freezetime->GetFloat();
	glinfo.iBombSitePlantingAt = -1;
	RespawnPlayers();
...
void RoundStart(void) {
	...
	if ( glinfo.iNumPlayersAliveMI6 == 0 || glinfo.iNumPlayersAlivePhoenix == 0 ) {
		...
		//Start the round
		glinfo.flRoundTimeEnd = gpGlobals->time + (mp_roundtime->GetFloat() * 60);
