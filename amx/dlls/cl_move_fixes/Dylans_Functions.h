...
struct entity_state_s //768 curstate
{
...
_inline int BF_GetRealBytesWritten(sizebuf_t_new *bf) { return bf->iCurBit >> 3; } // unpadded
qboolean BF_Overflow(sizebuf_t_new *bf, int nBits);
...
_inline int BF_GetNumBitsWritten(sizebuf_t_new *bf) { return bf->iCurBit; }
_inline int BF_GetNumBitsLeft(sizebuf_t_new *bf) { return bf->nDataBits - bf->iCurBit; }
qboolean BF_WriteBits(sizebuf_t_new *bf, const void *pData, int nBits);;
_inline byte *BF_GetData(sizebuf_t_new *bf) { return bf->pData; }
...
_inline int BF_GetNumBytesWritten(sizebuf_t_new *bf) { return BitByte(bf->iCurBit); }
...
struct trace_s
{
	MACBOOL allsolid; // if true, plane is not valid
	MACBOOL startsolid; // if true, the initial point was in a solid area
	MACBOOL inopen;
	MACBOOL inwater;
	int contents;
...
struct pmtrace_s
{
	MACBOOL allsolid; // if true, plane is not valid
	MACBOOL startsolid; // if true, the initial point was in a solid area
	MACBOOL inopen; // End point is in empty space or in water
	MACBOOL inwater;
	int contents;
	float fraction;  // time completed, 1.0 = didn't hit anything
	vec3_t endpos;  // final position
...
//static void (*GetViewAnglesFn)(float*) = (void(*)(float*))0x410A8950;
//static void (*SetViewAnglesFn)(float*) = (void(*)(float*))0x410A8954;

inline void GetViewAngles(float* angles)
{
	((void (*)(float*))( *(DWORD*)(0x410A8950)  ))(angles); 
}

inline void SetViewAngles(float* angles)
{
	((void (*)(float*))( *(DWORD*)(0x410A8954)  ))(angles); 
}
...
inline void NormalizeAngles(float* angles)
{
	for (int i = 0; i < 3; ++i)
	{
		angles[i] = fmodf(angles[i], 360.0f);
		if (angles[i] > 180.0f)
			angles[i] -= 360.0f;
		if (angles[i] < -180.0f)
			angles[i] += 360.0f;
