	...
	while ((pTarget = UTIL_FindEntityInSphere(pTarget, vecSrc, rad)) != NULL)
	{
		...
		if (pTarget->v.takedamage > DAMAGE_NO)
		{
...
#endif
			TRACE_LINE(vecSrc, pTarget->v.origin, 0 /*dylan*/, FALSE, pAttacker, &tr);
			...
			if(tr.flFraction < 1.0 || (hitId == targetId))
			{
				...
				{
					Vector vecPush = (pTarget->v.origin - (pAttacker->v.absmin + pAttacker->v.absmax) * 0.5);
					vecPush.NormalizeInPlace();
					if(dmg < 60.0)
					{
						//DYLAN pTarget->v.velocity = pTarget->v.velocity + vecPush * dmg * (sv_knockback1 ? sv_knockback1->value : 1.0f);
					} else {
						//DYLAN pTarget->v.velocity = pTarget->v.velocity + vecPush * dmg * (sv_knockback2 ? sv_knockback2->value : 1.0f);
					}
				}
