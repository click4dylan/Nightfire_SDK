...
static cell AMX_NATIVE_CALL get_pdata_cbase_safe(AMX *amx, cell *params)
{
	...
	void *ptr=*((void **)((int *)INDEXENT_NEW(index)->v.pvPrivateData + iOffset));
	...
	{
		if (ptr == INDEXENT_NEW(i)->v.pvPrivateData)
		{
			return i;
		}
	}
...
static cell AMX_NATIVE_CALL set_pdata_cbase(AMX *amx, cell *params)
{
	...
	if (target == -1)
	{
		*((void **)((int *)INDEXENT_NEW(index)->v.pvPrivateData + iOffset)) = NULL;
	}
	else
	{
		*((void **)((int *)INDEXENT_NEW(index)->v.pvPrivateData + iOffset)) = INDEXENT_NEW(target)->v.pvPrivateData;
	}
