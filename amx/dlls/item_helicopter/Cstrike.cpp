...
cvar_t* allow_helicopters = nullptr;
cvar_t* helicopter_health = nullptr;
...
void CItemHelicopter::Precache(void) {
	...
	PRECACHE_MODEL(PHELICOPTER_GLOW_SPRITE);
}
...
CItemHelicopterCamera::~CItemHelicopterCamera()
{
	...
	if (sprite) {
		REMOVE_ENTITY(sprite->edict());
		sprite = NULL;
	}
}

void CItemHelicopterCamera::KeyValue(KeyValueData *pkvd) {
	if (FStrEq(pkvd->szKeyName, "heliteam"))
	{
		team = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "speedmultiplier"))
	{
		speedmultiplier = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
...
#define SPRITE_CREATE(sprt, orgn, anim, unk) ((C_BaseEntity*(__cdecl*)(char* sprite, Vector& origin, BOOL animate, int unknown))0x4204FAE0)(sprt, orgn, anim, unk);
...
void CItemHelicopterCamera::Spawn(void) {
	...
	if (frametime > 0.0078125) {
		UPDATE_RATE = max(0.0001f, UPDATE_RATE - (frametime));
	}
	...
	sprite = SPRITE_CREATE(PHELICOPTER_GLOW_SPRITE, this->GetLocalOrigin() + Vector(0, 0, 3), TRUE, 4);
	if (sprite)
	{
		//PRECACHE_MODEL(PHELICOPTER_GLOW_SPRITE);
	//	SET_MODEL(spriteed, PHELICOPTER_GLOW_SPRITE);
		//sprite->pev->model = PRECACHE_MODEL(PHELICOPTER_GLOW_SPRITE);
		//sprite->pev->modelindex = PRECACHE_MODEL((char *)STRING(pev->model));
		//sprite->pev->classname = MAKE_STRING("env_sprite");
		//sprite->pev->movetype = MOVETYPE_NOCLIP;
		//sprite->pev->solid = SOLID_NOT;
		//sprite->pev->rendermode = kRenderGlow;
		//sprite->pev->renderfx = kRenderFxNone;
		//sprite->pev->renderamt = 255;
		//sprite->pev->rendercolor = Vector(0, 255, 0);
		//sprite->pev->aiment = this->edict();

		//Set transparency
		sprite->pev->rendermode = kRenderTransAdd;
		sprite->pev->renderfx = kRenderFxStrobeSlow;
		sprite->pev->renderamt = 255;

		if (team == TEAM_MI6)
			sprite->pev->rendercolor = Vector(0, 185, 255);
		else if (team == TEAM_PHOENIX)
			sprite->pev->rendercolor = Vector(245, 120, 0);
		else
			sprite->pev->rendercolor = Vector(255, 255, 255);

		//Set attachment
		sprite->pev->skin = ENTINDEX(this->edict());
		sprite->pev->body = 2;
		sprite->pev->aiment = this->edict();
		sprite->pev->movetype = MOVETYPE_FOLLOW;

		sprite->pev->scale = 0.25f;
		//sprite->pev->framerate = 2;
		sprite->pev->effects = EF_NODRAW;
		sprite->pev->spawnflags = 0x0001;
		//sprite->pev->frame = 0;
		sprite->pev->angles = Vector(0.0f, 0.0f, 0.0f);
		sprite->pev->origin = this->GetLocalOrigin();// +Vector(0, 0, 15);
		//sprite->pev->flags |= FL_CUSTOMENTITY;
		//sprite->Precache();
		//sprite->Spawn();
		//sprite->pev->aiment = this->edict();
	}
...
void CItemHelicopterCamera::Precache(void) {
	...
	PRECACHE_MODEL("sprites/muzzleflash6.spz");
	PRECACHE_MODEL(PHELICOPTER_GLOW_SPRITE);
...
void CItemHelicopterCamera::Use(CBaseEntityCustom *pActivator, CBaseEntityCustom *pCaller, USE_TYPE useType, float value) {
	...
	if (allow_helicopters->GetBool() == 0)
...
void CItemHelicopterCamera::Launch(void) {
	...
	m_flNextRestartSoundTime = gpGlobals->time + PHELICOPTER_RESTART_SOUND_TIME;
...
void CItemHelicopterCamera::FlyThink(void) {
	...
	if (team != 0 && pPlayer->team != team) {
		char teammsg[256];
		sprintf(teammsg, "This helicopter is only for team %s!", team == TEAM_MI6 ? "MI6" : "Phoenix");
		CLIENT_PRINTF(pPlayer->edict(), print_center, teammsg);
		LoseHelicopterControl();
		return;
	}
	...
	if (gpGlobals->time >= m_flNextRestartSoundTime)
	{
		StopSound(edict(), CHAN_AUTO, PHELICOPTER_SOUND);
		g_engfuncs.pfnEmitSound(edict(), CHAN_AUTO, PHELICOPTER_SOUND, VOL_NORM, ATTN_NORM, 0, PITCH_NORM);

		float dt = min(max(0.0f, gpGlobals->time - m_flNextRestartSoundTime), 0.1f);
		m_flNextRestartSoundTime = gpGlobals->time + PHELICOPTER_RESTART_SOUND_TIME - dt;
	}
	...
	if (cmd->buttons & IN_USE && newtime - time_pressedusekey > 1.0f){
		...
		LoseHelicopterControl();
		pPlayer->pev->takedamage = DAMAGE_AIM;
		pPlayer->TakeDamage(pPlayer->pev, pPlayer->pev, 300.0f, DMG_BULLET);
		return;
	}
		...
		if (newtime - lasttime_sloweddown >= UPDATE_RATE){
			...
			PHELICOPTER_SPEED = max(PHELICOPTER_SPEED - 10, PHELICOPTER_MIN_SPEED);
		}
	}
#if 0
	// ---------------------------------------------------
	...
	if (cmd->buttons & IN_MOVELEFT){
		...
		if (newtime - lasttime_movedleft >= UPDATE_RATE){
			...
			m_flRollSideSpeed = max(m_flRollSideSpeed - PHELICOPTER_ROLLSPEED_RATE, -PHELICOPTER_MAX_ROLLSPEED); //PHELICOPTER_MAX_ROLLSPEED / -1
		}
	}
	if (cmd->buttons & IN_MOVERIGHT){
		...
		if (newtime - lasttime_movedright >= UPDATE_RATE){
			...
			m_flRollSideSpeed = min(m_flRollSideSpeed + PHELICOPTER_ROLLSPEED_RATE, PHELICOPTER_MAX_ROLLSPEED);
		}
	}
	if (cmd->buttons & IN_JUMP || cmd->buttons & IN_FORWARD){
		//we are jumping or moving forward, move upwards
		if (newtime - lasttime_hoveredup >= UPDATE_RATE * 2){
			lasttime_hoveredup = newtime;
			if (!upsidedown){
				if (abs(angles.x) < 12){
					m_flUpSpeed = min(m_flUpSpeed + (5.0f * speedmultiplier), PHELICOPTER_MAX_UPSPEED * 1.2f); //move up
				}
				else{
					m_flUpSpeed = min(m_flUpSpeed + (1.0f * speedmultiplier), PHELICOPTER_MAX_UPSPEED); //move up
				}
			}
			else {
				m_flUpSpeed = max(m_flUpSpeed + (1.0f * speedmultiplier), -PHELICOPTER_MAX_UPSPEED); //we are upside down
			}
		}
	}
	if (cmd->buttons & IN_DUCK || cmd->buttons & IN_BACK){
		//we are ducking, move down.
		if (newtime - lasttime_hovereddown >= UPDATE_RATE * 2){
			lasttime_hovereddown = newtime;
			if (!upsidedown)
				m_flUpSpeed = max(m_flUpSpeed - (5.0f * speedmultiplier), -PHELICOPTER_MAX_UPSPEED); //move down
			else
				m_flUpSpeed = min(m_flUpSpeed - (1.0f * speedmultiplier), PHELICOPTER_MAX_UPSPEED); //we are upside down
		}
	}
	...
	if (!(cmd->buttons & IN_FORWARD) && !(cmd->buttons & IN_JUMP) && !(cmd->buttons & IN_DUCK) && !(cmd->buttons & IN_BACK)){
		if (newtime - lasttime_resetup > (UPDATE_RATE * 2)){
			...
			if (m_flUpSpeed < 0)
				m_flUpSpeed = min(m_flUpSpeed + 6, PHELICOPTER_MIN_SPEED);
			else if (m_flUpSpeed > 0)
				m_flUpSpeed = max(m_flUpSpeed - 6, PHELICOPTER_MIN_SPEED);
		}
	}
	if (!(cmd->buttons & IN_MOVELEFT) && !(cmd->buttons & IN_MOVERIGHT)){
		if (newtime - lasttime_resetleft > (UPDATE_RATE * 2)){
			...
			if (m_flRollSideSpeed < 0)
				m_flRollSideSpeed = min(m_flRollSideSpeed + 0.05, PHELICOPTER_MIN_SPEED);
			else if (m_flRollSideSpeed > 0)
				m_flRollSideSpeed = max(m_flRollSideSpeed - 0.05, PHELICOPTER_MIN_SPEED);
		}
	}
	...
	if (cmd->buttons & IN_ATTACK && newtime - lasttime_leftclicked >= RANDOM_FLOAT(0.125, 0.325)){
		...
		FIRE_BULLETS((CBaseEntity*)/*this*/pPlayer, 1, vecOrigin, vecDir, Vector(0.01f, 0.01f, 0), 4096.0f, 14, 1, 16, pPlayer->pev);
	...
	if (cmd->buttons & IN_ATTACK2 && newtime - lasttime_rightclicked >= 3.0f){
		if (rocketsshot < 2) {
			rocketsshot++;
			m_bShake = true;
	...
	if (angles.x > 25 && m_flUpSpeed > 10)
		m_flUpSpeed = max(0, m_flUpSpeed - 2);
	...
	vVelocity = vVelocity + (gpGlobals->v_up * m_flUpSpeed);
...
void CItemHelicopterCamera::FlyTouch(CBaseEntityCustom *pOther) {
	...
	//if (pOther->pev->solid == SOLID_BSP) {
		//pCamEdict->v.velocity = pev->velocity + gpGlobals->trace_plane_normal * (pev->velocity.Length() - 200);
		//pVisibleHeliModelEdict->v.velocity = pev->velocity + gpGlobals->trace_plane_normal * (pev->velocity.Length() - 200);
		//pev->velocity = pev->velocity + gpGlobals->trace_plane_normal * (pev->velocity.Length() - 200);
	//}
}
...
void OnModuleInitialize(void) {
	if (!allow_helicopters)
	{
		cvar_t_small allowhelicopters = { CVAR_BOOL, "allow_helicopters", "Allow item_helicopter", "1", FCVAR_EXTDLL | FCVAR_SERVER };
		cvar_t_small helicopterhealth = { CVAR_FLOAT, "helicopter_health", "item_helicopter health", "500.0", FCVAR_EXTDLL | FCVAR_SERVER };
		allow_helicopters = (cvar_t*)CVAR_REGISTER(allowhelicopters);
		helicopter_health = (cvar_t*)CVAR_REGISTER(helicopterhealth);
	}
...
void OnStartFrame(void) {
	...
	if (!glinfo.SpawnedHelisYet) {
		if (gpGlobals->mapname) {
			if (strcmpi(STRING(gpGlobals->mapname), "ctf_romania") || strstr(STRING(gpGlobals->mapname), "ctf_backstreets")) {
				glinfo.SpawnedHelisYet = true;
			}
			else {
				glinfo.SpawnedHelisYet = true;
				CItemHelicopterCamera *pCam = GetClassPtr((CItemHelicopterCamera *)NULL);
				if (strstr(STRING(gpGlobals->mapname), "ctf_romania")) {
				...
				else if (strstr(STRING(gpGlobals->mapname), "ctf_backstreets"))
				{
					SET_ORIGIN(pCam->edict(), Vector(1981, -5451, 556));
					pCam->pev->classname = MAKE_STRING("item_helicopter");
					pCam->pev->angles = g_vecZero;
					pCam->team = TEAM_PHOENIX;
					pCam->Spawn();
					...
					pCam = GetClassPtr((CItemHelicopterCamera *)NULL);
					SET_ORIGIN(pCam->edict(), Vector(2687, -911, 67));
					pCam->pev->classname = MAKE_STRING("item_helicopter");
					pCam->pev->angles = Vector(0, 180, 0);
					pCam->team = TEAM_MI6;
					pCam->Spawn();
