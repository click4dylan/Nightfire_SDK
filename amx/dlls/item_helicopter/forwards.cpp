...
{
	int retValue = OnAddToFullPack(state, e, ent, host, hostflags, player, pSet);
	if (retValue == 1)
...
void ClientCommand( edict_t *pEntity, int argl, const char *str ) {
	...
	if ( actualstring ) {
		OnClientCommand(pEntity, argl, actualstring);
	}
	RETURN_META(MRES_IGNORED);
}
